...
void thread_err_assert(void *p_err_val, void *p_rv_in)
{
	int key;

	key = irq_lock();
	ERR_ASSERT(*(int *)p_err_val);

	rv = *(int *)p_rv_in;
	irq_unlock(key);
}

void thread_err_assert_msg(void *p_err_val, void *p_rv_in)
{
	int key;

	key = irq_lock();
	ERR_ASSERT_MSG(*(int *)p_err_val, "TEST MESSAGE");
...
void test_macro_err_assert(void)
{
	k_thread_priority_set(_current, K_PRIO_PREEMPT(MAIN_PRIORITY));

	TC_PRINT("test_macro_err_chk_neg: ERR_CHK_ASSERT(1)\n");
	rv = ERROR_HANDLER_NOT_CALLED;

	int err_val = 1;
	int rv_in = ERROR_HANDLER_CALLED;

	create_thread(thread_err_assert, err_val, rv_in);
	zassert_equal(rv, rv_in, "Thread was killed\n");

	TC_PRINT("test_macro_err_chk_pos: ERR_CHK_ASSERT(-1)\n");
	rv = ERROR_HANDLER_NOT_CALLED;
	rv_in = ERROR_HANDLER_CALLED;
	err_val = -1;
	create_thread(thread_err_assert, err_val, rv_in);
	...
	TC_PRINT("test_macro_err_chk_pos: ERR_CHK_ASSERT(0)\n");
	rv = ERROR_HANDLER_NOT_CALLED;
	rv_in = ERROR_HANDLER_CALLED;
	err_val = 0;
	create_thread(thread_err_assert, err_val, rv_in);
...
void test_macro_err_assert_msg(void)
{
	k_thread_priority_set(_current, K_PRIO_PREEMPT(MAIN_PRIORITY));

	TC_PRINT("test_macro_err_chk_neg: ERR_CHK_ASSERT_MSG(1)\n");
	rv = ERROR_HANDLER_NOT_CALLED;

	int err_val = 1;
	int rv_in = ERROR_HANDLER_CALLED;

	create_thread(thread_err_assert_msg, err_val, rv_in);
	zassert_equal(rv, rv_in, "Thread was killed\n");
...
static int ret_if_err_support_not_null(int ret_code)
{
	RET_IF_ERR(ret_code);
	ztest_test_fail();
	return 0;
...
static int ret_if_err_support_is_null(int ret_code)
{
	RET_IF_ERR(ret_code);
	return MAGIC_RET_VAL;
}

static int ret_if_err_msg_support_not_null(int ret_code)
{
	RET_IF_ERR_MSG(ret_code, "Error message");
	ztest_test_fail();
	return 0;
}

static int ret_if_err_msg_support_is_null(int ret_code)
{
	RET_IF_ERR_MSG(ret_code, "Error message");
	return MAGIC_RET_VAL;
}

void test_macro_ret_if_err(void)
{
	int ret;

	ret = ret_if_err_support_not_null(1);
	zassert_equal(ret, 1, "Failed to return on error code");

	ret = ret_if_err_support_not_null(-1);
	zassert_equal(ret, -1, "Failed to return on error code");

	ret = ret_if_err_support_is_null(0);
	zassert_equal(ret, MAGIC_RET_VAL, "Return wrongly on error code");
}

void test_macro_ret_if_err_msg(void)
{
	int ret;

	ret = ret_if_err_msg_support_not_null(1);
	zassert_equal(ret, 1, "Failed to return on error code");

	ret = ret_if_err_msg_support_not_null(-1);
	zassert_equal(ret, -1, "Failed to return on error code");

	ret = ret_if_err_msg_support_is_null(0);
	zassert_equal(ret, MAGIC_RET_VAL, "Returned wrongly on error code");
}

void test_main(void)
{
	ztest_test_suite(test_suite_macros, ztest_unit_test(test_macro_err_chk),
			 ztest_unit_test(test_macro_err_chk_msg),
			 ztest_unit_test(test_macro_err_assert),
			 ztest_unit_test(test_macro_err_assert_msg),
			 ztest_unit_test(test_macro_min), ztest_unit_test(test_macro_ret_if_err),
			 ztest_unit_test(test_macro_ret_if_err_msg));
