...
#define USART2_IRQ_PREPRIO                      0x0F
...
static uint8_t uart_tx_buffer[USART2_TX_BUFFER_SIZE_BYTES];
static bsp_fifo_t uart_tx_fifo;
...
static void UART_Init(void)
{
    ...
    uart_drv_handle.Init.Mode         = UART_MODE_TX;
...
int __io_putchar(int ch)
{
    ...
    temp_in_index = uart_tx_fifo.in_index;

    // Wait while buffer is full
    do
    {
        __disable_irq();
        temp_out_index = uart_tx_fifo.out_index;
        __enable_irq();
    } while ((temp_in_index + 1) % uart_tx_fifo.size == temp_out_index);

    // Update input index and add char
    uart_tx_fifo.buffer[temp_in_index] = ch;
    uart_tx_fifo.in_index++;
    uart_tx_fifo.in_index %= uart_tx_fifo.size;

    // If UART is not transmitting, then kick off transmit
    if (temp_out_index == temp_in_index)
    {
        uint8_t *tx_buffer = uart_tx_fifo.buffer + uart_tx_fifo.out_index;

        HAL_UART_Transmit_IT(&uart_drv_handle, tx_buffer, 1);
    }
...
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *UartHandle)
{
    ...
    {
        // Update out_index
        uart_tx_fifo.out_index += UartHandle->TxXferSize;

        if (uart_tx_fifo.out_index >= uart_tx_fifo.size)
        {
            uart_tx_fifo.out_index = 0;
        }

        // Determine if buffer is not empty
        if (uart_tx_fifo.out_index != uart_tx_fifo.in_index)
        {
            // Calculate next TX size
            uint32_t tx_size_bytes = uart_tx_fifo.in_index;
            uint8_t *tx_buffer;

            if (uart_tx_fifo.in_index >= uart_tx_fifo.out_index)
            {
                tx_size_bytes = uart_tx_fifo.in_index - uart_tx_fifo.out_index;
            }
            else
            {
                tx_size_bytes = uart_tx_fifo.size - uart_tx_fifo.out_index;
            }

            tx_buffer = uart_tx_fifo.buffer + uart_tx_fifo.out_index;

            HAL_UART_Transmit_IT(UartHandle, tx_buffer, tx_size_bytes);
        }
    }
...
uint32_t bsp_initialize(bsp_app_callback_t cb, void *cb_arg)
{
    ...
    uart_tx_fifo.buffer = uart_tx_buffer;
    uart_tx_fifo.size = USART2_TX_BUFFER_SIZE_BYTES;
    uart_tx_fifo.in_index = 0;
    uart_tx_fifo.out_index = 0;
