...
#define INA_231_ALERT_PIN DT_GPIO_PIN(DT_NODELABEL(curr_mon_alert_in), gpios)
...
static int gpio_interrupt_init(void)
{
	...
	if (!gpio_dev) {
		LOG_ERR("device_get_binding(GPIO_1) error");
		return -EIO;
	}

	int gpio_flags = (GPIO_INPUT | GPIO_PULL_UP);

	ret = gpio_pin_configure(gpio_dev, INA_231_ALERT_PIN, gpio_flags);
	RET_IF_ERR(ret);

	gpio_init_callback(&gpio_cb, alert_pin_handler, BIT(INA_231_ALERT_PIN));

	ret = gpio_add_callback(gpio_dev, &gpio_cb);
	RET_IF_ERR(ret);

	ret = gpio_pin_interrupt_configure(gpio_dev, INA_231_ALERT_PIN, GPIO_INT_EDGE_TO_INACTIVE);
	RET_IF_ERR(ret);
