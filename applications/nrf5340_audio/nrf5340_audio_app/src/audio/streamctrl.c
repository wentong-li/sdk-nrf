...
static void m_button_evt_handler(struct button_evt event)
{
	...
	case BUTTON_PLAY_PAUSE:
		...
		} else if (NRF5340_AUDIO_DEV == NRF5340_AUDIO_DEV_GATEWAY) {
#if (CONFIG_TRANSPORT_BIS)
			switch (m_stream_state) {
			case STATE_DISCONNECTED:
				audio_gateway_start();
...
static void m_ble_transport_evt_handler(enum ble_evt_type event)
{
	...
	case BLE_EVT_DISCONNECTED:
		...
		case STATE_LINK_READY:
			stream_state_set(STATE_DISCONNECTED);
#if ((NRF5340_AUDIO_DEV == NRF5340_AUDIO_DEV_HEADSET) && CONFIG_TRANSPORT_BIS)
			ble_trans_iso_bis_rx_sync_get();
#endif /* ((NRF5340_AUDIO_DEV == NRF5340_AUDIO_DEV_HEADSET) && CONFIG_TRANSPORT_BIS) */
			break;
...
#endif /* (NRF5340_AUDIO_DEV == NRF5340_AUDIO_DEV_HEADSET) */
#if ((NRF5340_AUDIO_DEV == NRF5340_AUDIO_DEV_GATEWAY) && CONFIG_TRANSPORT_CIS)
			stream_state_set(STATE_DISCONNECTED);
			audio_stop();
#endif /* ((NRF5340_AUDIO_DEV == NRF5340_AUDIO_DEV_GATEWAY) && CONFIG_TRANSPORT_CIS) */
			break;
